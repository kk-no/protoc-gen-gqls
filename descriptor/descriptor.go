...
func Load(req *plugin.CodeGeneratorRequest) (*plugin.CodeGeneratorResponse, error) {
	...
	for _, name := range req.GetFileToGenerate() {
		...
		go func() {
			defer wg.Done()
			content.service = getService(f.GetPackage(), f.GetService())
		}()

		wg.Add(1)
		go func() {
			defer wg.Done()
			content.message = getMessage(f.GetPackage(), f.GetMessageType())
		}()

		wg.Add(1)
		go func() {
			defer wg.Done()
			content.enum = getEnum(f.GetPackage(), f.GetEnumType())
		}()
		...
		res.File = append(res.File, &plugin.CodeGeneratorResponse_File{
			...
			Content: proto.String("# Generated by protoc-gen-gqls." + content.String()),
		})
	}
